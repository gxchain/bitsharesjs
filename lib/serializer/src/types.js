  ...
  appendByteBuffer(b, object) {
    ...
    var len =
      typeof object === 'string'
        ? ByteBuffer.calculateUTF8Bytes(object)
        : object.length;
    b.writeVarint32(len);
...
Types.void = {
  fromByteBuffer(b) {
    throw new Error('(void) undefined type', b);
  },
  appendByteBuffer(b, object) {
    throw new Error('(void) undefined type', b, object);
  },
  fromObject(object) {
    console.warn('(void) undefined type', object);
    return object;
    // throw new Error('(void) undefined type', object);
  },
  toObject(object, debug = {}) {
    if (debug.use_default && object === undefined) {
      return undefined;
    }
    console.warn('(void) undefined type');
    return object;
...
export const string_to_name = (str) => {
  ...
  for (; i < len && i < 12; ++i) {
    ...
    let symbol = v
      .to_long(char_to_symbol(str[i].charCodeAt(0)))
      .toUnsigned()
      .and(0x1f);
